...
const Dashboard = (props: { children: ReactNode }) => {
  ...
  const modules = [
    { id: 1, name: "Users", path: "/admin/users", icon: <User size={50} color="#ffffff" /> },
    { id: 2, name: "Requests", path: "/admin/requests", icon: <Send size={50} color="#ffffff" /> },
    { id: 3, name: "Home", path: "/admin", icon: <ShieldHalf size={50} color="#ffffff" /> },
  ];
            ...
            <div className="p-3 w-full h-full flex flex-col justify-center items-center">
              {modules.map((module) => (
                ...
                >
                    ...
                    <p className="text-white text-2xl font-bold">{module.name}</p>
                </div>
              ))}
            </div>
          </div>
